            ...
            <div class="card shadow">
                <div class="card-body">
                    <div class="table-header">
                        <h4>{{ __('Edit Tax Group') }}</h4>
                    </div>

                    <div class="order-form-section p-16">
                        {{-- form start --}}
                        <form action="{{ route('business.taxes.update',$tax->id) }}" method="post" enctype="multipart/form-data"
                                    ...
                                    <div class="col-md-6 mt-2">
                                        ...
                                        <div class="input-group">
                                            <select id="sub_tax" name="tax_ids[]" class="form-control" multiple>
                                                @php
                                                    $selectedtaxIds = collect($tax->sub_tax)->pluck('id')->toArray();
                                                @endphp

                                                @foreach ($taxes as $tax_item)
                                                    <option value="{{ $tax_item->id }}" @selected(in_array($tax_item->id, $selectedtaxIds))>
                                                        {{ $tax_item->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="mt-2 col-lg-6">
                                        ...
                                        <div class="gpt-up-down-arrow position-relative">
                                            <select class="form-control form-selected" name="status">
                                                <option value="1" @selected($tax->status == 1)>{{ __('Active') }}</option>
                                                <option value="0" @selected($tax->status == 0)>{{ __('Deactive') }}</option>
                                            </select>
                                            <span></span>
                                        </div>
                                    </div>
