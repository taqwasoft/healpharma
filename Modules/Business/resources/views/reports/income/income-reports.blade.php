        ...
        <div class="mb-4 d-flex loss-flex  gap-3 loss-profit-container d-print-none">
            <div class="d-flex align-items-center justify-content-center gap-3">
                <div class="profit-card p-3 text-white">
                    ...
                    <p class="stat-value" id="total_income">{{ currency_format($total_income, 'icon', 2, business_currency()) }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="card-bodys">
                ...
                <div class="table-top-form ">
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <form action="{{ route('business.income-reports.filter') }}" method="post" class="report-filter-form" table="#income-reports-data">
                            @csrf
            ...
            <div class="responsive-table table-container">
                <table class="table" id="datatable">
                    ...
                    <tr>
                        ...
                        <th class="text-start table-header-content">{{ __('Amount') }}</th>
                        <th class="text-start table-header-content">{{ __('Category') }}</th>
                        <th class="text-start table-header-content">{{ __('Income For') }}</th>
                        <th class="text-start table-header-content">{{ __('Payment Type') }}</th>
                        <th class="text-start table-header-content">{{ __('Reference Number') }}</th>
                        <th class="text-start table-header-content">{{ __('Income Date') }}</th>
                    </tr>
                    </thead>
                    <tbody id="income-reports-data">
