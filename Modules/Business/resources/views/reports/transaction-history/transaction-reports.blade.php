            ...
            <div class="row">
                <div class="col-lg-2 col-md-12 ">
                    <div class="loss-card p-3 m-2 text-white">
                        ...
                        <p class="stat-value" id="total_due">{{ currency_format($total_due, 'icon', 2, business_currency()) }}</p>
                    </div>
                </div>
                <div class="col-lg-2 col-md-6 ">
                    <div class="profit-card p-3 m-2 text-white">
                        <p class="stat-title">{{ __('Paid') }}</p>
                        <p class="stat-value" id="total_paid">{{ currency_format($total_paid, 'icon', 2, business_currency()) }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mt-4">
            <div class="card-bodys">
                ...
                <div class="table-header justify-content-center border-0 d-none d-block d-print-block  text-center">
                    ...
                    <h4 class="mt-2">{{ __('Due Transactions') }}</h4>
                </div>

                <div class="table-top-form">
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <form action="{{ route('business.transaction-history-reports.filter') }}" method="post" class="report-filter-form" table="#transactions-reports-data">
                            @csrf
            ...
            <div class="responsive-table table-container">
                <table class="table" id="datatable">
                    ...
                    <tr>
                        ...
                        <th class="text-start table-header-content">{{ __('Invoice Number') }}</th>
                        <th class="text-start table-header-content">{{ __('Party Name') }}</th>
                        <th class="text-start table-header-content">{{ __('Total Due') }}</th>
                        <th class="text-start table-header-content">{{ __('Pay Due Amount') }}</th>
                        <th class="text-start table-header-content">{{ __('Payment Type') }}</th>
                        <th class="text-start table-header-content">{{ __('Payment Date') }}</th>
                    </tr>
                    </thead>
                    <tbody id="transactions-reports-data">
